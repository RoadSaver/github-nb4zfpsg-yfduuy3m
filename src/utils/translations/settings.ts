export const settings = {
  en: {
    'settings': 'Settings',
    'configure-preferences': 'Configure your preferences',
    'account': 'Account',
    'account-info': 'Account information',
    'payment': 'Payment',
    'payment-methods': 'Payment methods',
    'payment-options': 'Payment options',
    'add-payment-method': 'Add payment method',
    'payment-options-desc': 'Payment options will be available in a future update',
    'history': 'History',
    'request-history': 'Request history',
    'requests-desc': 'View all your previous service requests',
    'about': 'About',
    'app-info': 'App information',
    'no-requests': 'No requests found',
    'completed': 'Completed',
    'declined': 'Declined',
    'employee': 'Employee',
    'total-price': 'Total price',
    'address': 'Address',
    'view-on-map': 'View on map',
    'reason': 'Reason',
    'username': 'Username',
    'email': 'Email',
    'phone': 'Phone number',
    'gender': 'Gender',
    'joined-date': 'Member since',
    'edit-profile': 'Edit profile',
    'change-password': 'Change password',
    'logout': 'Logout',
    'logout-confirm': 'Are you sure you want to logout?',
    'cancel': 'Cancel',
    'confirm': 'Confirm',
    'save-changes': 'Save changes',
    'profile-updated': 'Profile updated',
    'profile-update-success': 'Your profile has been updated successfully',
    'profile-update-error': 'Failed to update profile. Please try again',
    'password-updated': 'Password updated',
    'password-update-success': 'Your password has been updated successfully',
    'password-update-error': 'Failed to update password. Please try again',
    'current-password': 'Current password',
    'new-password': 'New password',
    'confirm-new-password': 'Confirm new password',
    'work-hours-line1': 'Monday to Friday: 09:00 - 17:00',
    'work-hours-line2': 'Saturday and Sunday: Closed',
    'outside-hours-contact': 'For service outside working hours, contact support',
    'payment-future-update': 'Payment options will be available in a future update',
    'external-link': 'Open public app link',
  },
  bg: {
    'settings': 'Настройки',
    'configure-preferences': 'Конфигурирайте вашите предпочитания',
    'account': 'Акаунт',
    'account-info': 'Информация за акаунта',
    'payment': 'Плащане',
    'payment-methods': 'Методи за плащане',
    'payment-options': 'Опции за плащане',
    'add-payment-method': 'Добави метод на плащане',
    'payment-options-desc': 'Опциите за плащане ще бъдат налични в бъдеща актуализация.',
    'history': 'История',
    'request-history': 'История на заявките',
    'requests-desc': 'Вижте всички ваши предишни заявки за услуги',
    'about': 'За нас',
    'app-info': 'Информация за приложението',
    'no-requests': 'Няма намерени заявки',
    'completed': 'Завършен',
    'declined': 'Отхвърлен',
    'employee': 'Служител',
    'total-price': 'Обща цена',
    'address': 'Адрес',
    'view-on-map': 'Виж в картата',
    'reason': 'Причина',
    'username': 'Потребителско име',
    'email': 'Имейл',
    'phone': 'Телефонен номер',
    'gender': 'Пол',
    'joined-date': 'Член от',
    'edit-profile': 'Редактиране на профила',
    'change-password': 'Смяна на парола',
    'logout': 'Изход',
    'logout-confirm': 'Сигурни ли сте, че искате да излезете?',
    'cancel': 'Отказ',
    'confirm': 'Потвърждение',
    'save-changes': 'Запазване на промените',
    'profile-updated': 'Профилът е актуализиран',
    'profile-update-success': 'Профилът ви беше актуализиран успешно.',
    'profile-update-error': 'Неуспешна актуализация на профила. Моля, опитайте отново.',
    'password-updated': 'Паролата е актуализирана',
    'password-update-success': 'Паролата ви беше актуализирана успешно.',
    'password-update-error': 'Неуспешна актуализация на паролата. Моля, опитайте отново.',
    'current-password': 'Текуща парола',
    'new-password': 'Нова парола',
    'confirm-new-password': 'Потвърдете новата парола',
    'work-hours-line1': 'Понеделник до петък: 09:00 - 17:00',
    'work-hours-line2': 'Събота и неделя: Почивни дни',
    'outside-hours-contact': 'За услуги извън работно време, свържете се с поддръжка',
    'payment-future-update': 'Опциите за плащане ще бъдат налични в бъдеща актуализация',
    'external-link': 'Отвори публичния линк на приложението',
  },
};
